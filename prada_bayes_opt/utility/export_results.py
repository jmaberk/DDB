...
def print_result_ystars(baysOpt,myfunction,Score,mybatch_type,acq_type,acq_params,toolbox='GPyOpt'):
...
#            print 'BatchSz={:.3f}({:.2f})'.format(np.mean(SumBatch),np.std(SumBatch))
    ...
    if 'BatchSz' in Score:
        ...
        if acq_type['name']=='lei': # if ELI acquisition function, we have extra variable 'k'
            ...
            strFile="{:s}_{:d}_{:s}_{:s}_{:s}.pickle".format(myfunction.name,myfunction.input_dim,mybatch_type,acq_type['name'],acq_params['expandSS'])
        
        path=os.path.join(out_dir,strFile)
        with open(path, 'w') as f:
            pickle.dump([ybest, MyTime,baysOpt.scalebounds,MyOptTime,ystars], f)
