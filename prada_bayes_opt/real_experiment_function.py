...
class SVR_function:
    ...
    def func(self,X):
        X=np.asarray(X)
        ##########################CHANGE PATH##################################    
        Xdata, ydata = self.get_data("C:\Users\jmberk\OneDrive\Documents\PhD\Code\Bayesian\\PradaBayesianOptimization\\real_experiment\\space_ga_scale")
...
#            x4=X[:,3]
        ...
        eng = matlab.engine.start_matlab()
        ##########################CHANGE PATH##################################
        eng.addpath(r'C:\Users\jmberk\OneDrive\Documents\PhD\Code\Bayesian\PradaBayesianOptimization\real_experiment\KWN_Heat_Treatment',nargout=0)
...
class DeepLearning_MLP_MNIST_5D:
    ...
    def __init__(self,  bounds=None,sd=None):
        ...
        if bounds == None:  # n_node: 512, dropout 0.2, 512, 0.2, 10 # learning rate, decay, momentum
            self.bounds = OrderedDict([('dropout1',(0.01,0.5)),('dropout2',(0.01,0.5)),
    ...
    def run_MLP_MNIST(self,X,X_train,Y_train,X_test,Y_test):
        ...
        sgd = SGD(lr=X[2], decay=X[3], momentum=X[4], nesterov=True)
    ...
    def func(self,X):
        X=X/max_bound_size
        if X.ndim==1:
            X[0]=X[0]*(0.5-0.01)+0.01
            X[1]=X[1]*(0.5-0.01)+0.01
            X[2]=X[2]*(1-0.01)+0.01
            X[3]=X[3]*(1e-5-1e-8)+1e-8
            X[4]=X[4]*(1-0.5)+0.5
...
class Robot_BipedWalker:
    ...
    def run_BipedWalker(self,X):
        ...
        eng = matlab.engine.start_matlab()
        ##########################CHANGE PATH##################################
        eng.addpath(r'C:\Users\jmberk\OneDrive\Documents\PhD\Code\Bayesian\PradaBayesianOptimization\real_experiment\WGCCM_three_link_walker_example\WGCCM_three_link_walker_example',nargout=0)
...
class BayesNonMultilabelClassification:
    ...
    def __init__(self,  bounds=None,sd=None):
        ...
        if bounds == None:  
            self.bounds = OrderedDict([('eta_xx',(0.001,0.1)),('eta_yy',(0.001,0.1)),('svi_rate',(0.0001,0.1)),('lambda',(1,500)),
                                        ('trunc',(0.0001,0.1)),('alpha',(0.1,5))])
        else: 
            self.bounds = bounds
    ...
    def run_BNMC(self,X):
        #print X
        ##########################CHANGE PATH##################################
        eng.addpath(r'C:\Users\jmberk\OneDrive\Documents\PhD\Code\Bayesian\PradaBayesianOptimization\real_experiment\BNMC\utilities',nargout=0)
        eng.addpath(r'C:\Users\jmberk\OneDrive\Documents\PhD\Code\Bayesian\PradaBayesianOptimization\real_experiment\BNMC\data',nargout=0)
        eng.addpath(r'C:\Users\jmberk\OneDrive\Documents\PhD\Code\Bayesian\PradaBayesianOptimization\real_experiment\BNMC',nargout=0)
...
#        eng.addpath(r'D:\OneDrive\Documents\PhD\Code\Bayesian\PradaBayesianOptimization\real_experiment\BNMC',nargout=0)
    ...
    def func(self,X):
        ...
        X=X/max_bound_size
        if X.ndim==1:
            X[0]=X[0]*(0.1-0.001)+0.001
            X[1]=X[1]*(0.1-0.001)+0.001
            X[2]=X[2]*(0.1-0.0001)+0.0001
            X[3]=X[3]*(500-1)+1
            X[4]=X[4]*(0.1-0.0001)+0.001
            X[5]=X[5]*(5-0.1)+0.1 
        else:
            X[:,0]=X[:,0]*(0.1-0.001)+0.001
            X[:,1]=X[:,1]*(0.1-0.001)+0.001
